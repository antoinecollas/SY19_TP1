...
## Régression linéaire
Nous commençons par vérifier les hypothèses de normalité et d'homoscédasticité. 
```{r}
reg <- lm(formula = y~., data=data_train)
```

appliquer une régression linéaire sur toutes les variables.
...
row.names(res) <- c("Régression linéaire")
#res[1,] <- CV_eval(lm, data_train)
```
