...
##Validation croisée
Nous avons implémenté une fonction qui calcule l'erreur de classification et l'écart type par validation croisée pour l'ensemble des modèles que nous utiliserons par la suite, et les calcules aussi pour le modèle utilisant le meilleur sous-ensemble de prédicteur à chaque étape de la validation croisée.
```{r, include=FALSE}
source('CV2.R')
```
...
##Analyses discriminantes (linéaire, quadratique, k-plus proches voisins et classifieur bayésien naïf).
Nous commençons par utiliser les analyses discriminantes linéaire et quadratique, la méthode des k-plus proches voisins ainsi que le classifieur bayésien naif.
```{r}
set.seed(123)
...
row.names(res) <- c("ADL", "ADL avec subset", "ADQ", "ADQ avec subset", "Bayésien naif", "Bayésien naif avec subset", "KNN", "KNN avec subset")
```
...
Dans le cas de la méthode des k-plus proches voisins, l'erreur et l'écart type correspondent à ceux dont le nombre de voisin k minimise au maximum l'érreur.
...
res <- suppressWarnings(CV_eval2(c('reg_logistique'), data_train_glm))
colnames(res) <- c("Erreur", "Ecart type")
row.names(res) <- c("Logistique", "Logistique avec subset")
```
...
Une deuxième méthode de régulariser nos modèles (ADL, ADQ, regression logistique) est de les entrainer avec un sous ensemble de variables. En réduisant la dimensionnalité en entrée, nous réduisons fortement le nombre de paramètres (réduction quadratique pour ADL et ADQ) tout en essayant de conserver un maximum de l'information initiale. Pour cela nous utilisons une analyse en composantes principales.
...
```
```{r, fig.height = 3, fig.width = 5, fig.align = "center"}
res <- data.frame("Erreur ADQ"=rep(0,ncol(Z)), "Ecart type ADQ"=rep(0,ncol(Z)), "Erreur naif bayésien"=rep(0,ncol(Z)), "Ecart type naif bayésien"=rep(0,ncol(Z)))
...
for (i in 1:ncol(Z)){
  ...
  res[i,] <- c(CV_eval('adq', data = data_train_pca), CV_eval('bayesien_naif', data = data_train_pca))
}
